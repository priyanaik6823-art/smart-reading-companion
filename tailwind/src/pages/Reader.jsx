...
const Reader = () => {
  ...
  // NEW: Responsive PDF width
  const [pdfWidth, setPdfWidth] = useState(700);

  useEffect(() => {
    const updateWidth = () => {
      if (window.innerWidth < 768) {
        setPdfWidth(window.innerWidth - 30);
      } else {
        setPdfWidth(700);
      }
    };
  ...
  const saveProgress = async (page) => {
    try {
      await axios.post(
        "https://smart-reading-companion-1.onrender.com/api/progress/save",
        {
          userId,
          bookId,
          currentPage: page,
  ...
  return (
    <div className="min-h-screen bg-[#121212] text-white flex flex-col items-center px-3 py-4">
      ...
      {/* PDF VIEWER */}
      <div className="bg-white rounded-xl p-2 max-w-full overflow-hidden">
        <Document file={pdfUrl} onLoadSuccess={onDocumentLoadSuccess}>
